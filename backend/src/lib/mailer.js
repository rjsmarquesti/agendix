  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#111">
        ...
        <p>Seu período de teste de <strong>14 dias</strong> já começou. Siga os passos abaixo para aproveitar ao máximo:</p>
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#111">
        ...
        <p>Olá, <strong>${nome}</strong>!</p>
        <p>Você ainda tem <strong>11 dias de teste gratuito</strong> no Agendix. Mas para aproveitar de verdade, precisa de 3 passos simples:</p>
...
async function enviarEmailTrialD10({ para, nome, slug }) {
  ...
  await transporter.sendMail({
    from: `"Agendix" <${from}>`,
    to: para,
    subject: `Faltam 4 dias para o seu teste encerrar — ${nome}`,
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#111">
        <h2 style="color:#d97706;margin-bottom:4px">⏳ Seu teste termina em 4 dias</h2>
        <p>Olá, <strong>${nome}</strong>!</p>
        <p>Seu período de teste no Agendix encerra em <strong>4 dias</strong>. Após isso, o acesso será suspenso até a escolha de um plano.</p>
